...
SELECT * FROM  TBL_MEMBER WHERE NAME in('김모모','박나연','김나현');
...
-- 8) mno 컬럼값으로 조회
-- mno값이 4미만 조회
SELECT mno FROM TBL_MEMBER WHERE mno < 4;


-- mno 값이 1,2,5,7 이 아닌것 조회 : 
SELECT mno FROM TBL_MEMBER WHERE mno NOT IN(1,2,5,7);


-- mno 값이 3~6 인 것 조회  : between (AND 연산)
SELECT mno FROM TBL_MEMBER WHERE mno BETWEEN 3 AND 6;


-- 9) 이름 순서로 모든 행과 열 조회, * : 모든 컬럼을 지정하는 문자(와일드문자) 
