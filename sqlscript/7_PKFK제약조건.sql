/* `제약 조건 constraints`
   ...
   1) NOT NULL(널 허용x)   2) UNIQUE(유일한 값)    3) CHECK (값의 범위)
   3) PRIMARY KEY (기본키) : 1) 과 2)를 모두 만족. **데이터 행을 식별(구별)
   4) FOREIGN KEY(외래키, 참조키) : 다른 테이블의 값을 참조하기 위해 사용
*/   
...
--  PK 설정방법 2) 테이블레벨	
	...
	REFERENCES 
			tbl_student(stuno)   -- 참조관계(하는) 테이블 설정
);	
/*
...
-		 하며 참조컬럼은 `학생테이블의 stuno` 이고 , `외래키 컬럼`은 성적테이블의 stuno 입니다.	
*/


-- 필요시 참조관계일때 테이블 삭제 
-- 참조 관계 부모테이블 `학생`은 DROP 사용 불가능
-- 2개 테이블을 모두 삭제하려면 자식 테이블을 먼저 DROP 하고 부모테이블 DROP 순서로 실행 
DROP TABLE TBL_STUDENT ;	-- 부모테이블 (먼저 삭제될 수 없다)
DROP TABLE TBL_SCORE ;	


-- 필요시 참조관계일때 테이블 데이터 삭제 
TRUNCATE TABLE TBL_STUDENT;	-- 참조되는 데이터가 자식 테이블에 있으면 부모 테이블 데이터 삭제 불가능
...
-- PK(행을 `식별`하는 역할) 컬럼을 조건으로 
-- select 조회하면 조회되는 행의 개수는 항상 0 또는 1입니다.
...
--PK 가 여러개 컬럼으로 구성(복합 컬럼)될 때에는 컬럼들을 and 조건으로 작성하여 행을 식별 할 수 있습니다.
...
SELECT * FROM TBL_SCORE ts WHERE stuno = '2019019';			-- N개 보임(PK로 조회하지 않아서)
...
- 성적테이블에 '과목명'을 학생 1명이 동일한 과목을 수강한다면(예를 들면 1학년 국어 ,  2학년 국어, 3학년 국어 )
	...
	3) 테이블은 최소로 데이터가 중복되도록 구성합니다. 데이터 중복은 데이터의 불일치를 발생시킵니다
		테이블을 구성하는 과정을 `정규화`	라고 합니다. 정규화가 잘 되어있다 => 테이블 구성을 잘했다.  
*/
