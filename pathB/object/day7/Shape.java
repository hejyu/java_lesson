...
/*  클래스의 상속 - 자바에서 제공하는 클래스들의 상속관계를 이해하기 위함. 직접 만드는 것은 몇 가지 기능으로 제한되어 있음
...
public class Shape {    
		...
		// 도형의 너비가 MAX_WIDTH를 넘으면 도화지가 작아서 그림을 못 그램 등등 
		// static final int 상수  = > 기준을 정하는 하나의 예시
		// 상수는 상속의 의미가 없음
		...
		protected String etc;			//protected : 자식 클래스한테만 오픈된 필드
		int round;						//접근 한정자가 없는 경우
		
		public Shape() {
			// 아래 커스텀 생성자가 있을 때 기본생성자도 사용하고 싶으면 직접 정의하기
		...
		public void printInfo() {
			System.out.print("super 메소드 테스트-"  + shapeName + "의 너비:" + width +",높이:" 
					+height);
